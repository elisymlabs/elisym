      ...
      const unexplained = mine
        .filter(
          (entry) =>
            ...
            // Free jobs included. Retry on one costs no money, but it costs the
            // customer the answer - the same deterministic refusal, with no
            // reason on screen either time - and the round trip is already being
            // made for the paid entries in the same batch. The cap below is what
            // bounds it.
