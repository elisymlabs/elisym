...
export class FileSettlementStore implements SettlementStore {
  ...
  claim(signature: string, jobIdentity: string): SettlementClaim {
    ...
    for (const [previous, record] of Object.entries(file.settlements)) {
      // `previous !== signature` is PROVABLY REDUNDANT for every ANSWER this
      // store gives: the line below re-adds the same key immediately, so
      // deleting it first changes nothing a reader can observe. NOT KILLED BY
      // ANY TEST - though one could be written, since the delete-and-re-add
      // moves the key to the end of the object and a fixture asserting key
      // ORDER would see that. Kept because a loop that can delete the key it is
      // about to write reads like a bug even when it is not.
