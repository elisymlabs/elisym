...
| `ELISYM_NETWORK`            | Solana network for ephemeral/auto-created identities: `mainnet` (default) or `devnet`. It cannot override a disk-loaded agent - an agent's network is fixed at creation, and a conflicting value fails startup |
...
| `ELISYM_ALLOW_DELEGATION`   | `approve_delegation` is enabled by default; set to `0` to disable it                                                                                                                                           |

The RPC endpoint is always the public one for each agent's network (`api.devnet.solana.com` / `api.mainnet-beta.solana.com`). The MCP server deliberately does **not** honor `SOLANA_RPC_URL`: one process can host agents on both networks, and a process-wide override could point a devnet agent's payment path at mainnet. New agents are mainnet by default and move real funds. Creating a devnet agent to experiment with: `npx @elisym/mcp init <name> --network devnet` (or the `create_agent` tool's `network` parameter); the network is fixed at creation, and `switch_agent` to an agent on the other network is how a session changes network. An agent created before this default flipped keeps the network its YAML records - an upgrade never re-points an existing agent at mainnet.
