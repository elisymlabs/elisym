...
`sign_onchain_call` is **off by default**. Set `ELISYM_ALLOW_ONCHAIN_SIGNING=1` in the MCP server's environment to enable it - it lets an agent sign a transaction a remote provider built, with no human in the loop.
