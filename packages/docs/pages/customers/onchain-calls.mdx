...
There is no human in that loop, so two rules are stricter than in the browser. A call that changes **who controls a token account that is not your wallet's own, or a token mint** - a new owner, delegate or close authority, a freeze, a changed mint or freeze authority - is refused outright unless you pass `accept_unattributed`: no ordinary swap does that to a pool's vault or to a mint, and it is what a takeover of funds a program holds for you, or of a token you issued, looks like. It covers token accounts and mints only: a change of who controls a stake account or a lending record is program state elisym cannot read, and stays in the notice. Accounts a call merely **writes to** - a pool's state, a pool vault that loses what the swap pays you - are listed in the preview as a notice instead: they are there on practically every routed swap, so refusing on them made the override a reflex, and elisym cannot tell a pool vault from an escrow holding your deposit. Read that list when the capability has no business touching your positions. And what the call moves - an approval counted at the amount it **authorizes**, not the zero it moves today, plus the SOL the fee and any account rent cost - is charged against your [session spend limits](/customers/mcp).
