...
- `submit_and_pay_job_from_file` - same, but the input comes from a file on disk (keeps large inputs out of the model's token budget). Like `submit_and_pay_job`, it pays the listed price up front.
...
- `submit_delegated_job` - submit a job paid from your existing [spl-approve delegation](/providers/delegated-execution#delegated-job-payment): the provider does the work first, then pulls from your delegated USDC allowance - no per-job payment transaction from you. On an ordinary capability that is the advertised price; on a [metered](/providers/metered-pricing) one the advertised price is a ceiling and the pull is what the job actually used. Requires an active delegation to the delegate key the capability advertises (`get_delegation` to check, `approve_delegation` to grant). The tool verifies the delegation, cap, and balance before publishing, asks for price confirmation via `max_price_lamports`, and signs a single-use, short-lived proof with your wallet key; the result reports the provider's pull transaction. An input too large to ride inline is sent over [iroh](/customers/files) automatically.
- `submit_delegated_job_from_file` - same as `submit_delegated_job`, but the input comes from a file on disk and travels peer-to-peer over [iroh](/customers/files), with an optional inline `prompt`. Use it instead of `submit_and_pay_job_from_file` when a capability advertises delegation: a metered card then bills only what the job used rather than the listed price.
