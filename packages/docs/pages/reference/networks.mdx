...
`mainnet` and `devnet` are elisym's two **environments**. A chain inside an environment is named by its [CAIP-2](https://chainagnostic.org/CAIPs/caip-2) id; the SDK registry (`CHAINS`, see [SDK payments](/sdk/payments#chains-and-evm-coins)) holds them:

| Chain slug | Environment | CAIP-2 id                                 | Coins the SDK knows        |
| ---------- | ----------- | ----------------------------------------- | -------------------------- |
| `solana`   | mainnet     | `solana:5eykt4UsFv8P8NJdTREpY1vzqKqZKvdp` | SOL, USDC, LSM             |
| `solana`   | devnet      | `solana:EtWTRABZaYq6iMfeYKouRu166VU2xqa1` | SOL, USDC                  |
| `tempo`    | mainnet     | `eip155:4217`                             | USDC.e (default), pathUSD  |
| `tempo`    | devnet      | `eip155:42431` (Moderato)                 | pathUSD                    |

Tempo has no native coin: gas is paid in a stablecoin. USDC.e is `0x20c000000000000000000000b9537d11c60e8b50`; pathUSD is `0x20c0000000000000000000000000000000000000` on both Tempo networks. Both have 6 decimals.
