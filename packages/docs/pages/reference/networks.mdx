...
The `elisym-config` program is deliberately deployed at the **same address** on both clusters; the protocol fee at mainnet launch is **0 bps**, configurable on-chain (clients read the live config, so a change needs no client release). Both USDC mints are 6 decimals.
...
- **Drop any explicit `mint:` field.** A bare `token: usdc` resolves to the correct mint for the agent's network automatically. An explicit mint must be canonical for the agent's network - a wrong-network mint fails loud at skill load rather than publishing an unpayable card.
