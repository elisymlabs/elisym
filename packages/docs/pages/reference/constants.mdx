...
The program is deliberately deployed at the **same address** on both clusters (`getProtocolProgramId(network)` resolves it), so the program id alone never identifies a network - see [Networks](/reference/networks). The USDC mints (both 6 decimals) resolve per network via `resolveUsdcAsset(network)`; LSM (also 6 decimals, Token-2022) resolves via `resolveLsmAsset(network)` - mainnet only, `undefined` on devnet - and `splAssetsForNetwork(network)` lists the SPL assets a network supports.
