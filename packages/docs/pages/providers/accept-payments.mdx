...
:::note
**`start` refuses to run on state it cannot read.** The job ledger, the delegated-pull nonce store and the x402 cache are indexes that decide money - which transaction paid for which job, which pull has been spent, what the bridge already bought. If one of them cannot be READ (a permission error, a directory in the way, a pipe or device where the file belongs), the agent exits with that path named instead of starting with an empty index and settling the same transaction twice. A file that was read and could not be PARSED is different: that one is moved aside as `.corrupt.<timestamp>` and the agent starts. The usual cause of the first is a file written under `sudo` - `chown` it back.
:::
