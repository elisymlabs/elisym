...
- **exit 43 is still the right code.** A 43 with no file is handled as the failure it looks like, and the operator log says the contract was not honoured; a file written without 43 is still a refusal.
