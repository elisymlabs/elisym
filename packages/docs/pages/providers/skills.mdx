...
| `43`          | understood the request and refuses it  | none; **stdout is sent to the customer as the reason** |
...
Exit `43` (`SCRIPT_EXIT_REFUSED`) is that channel, and it works for `dynamic-script`, `static-script` and `onchain`:
...
- **stdout** is what the buyer reads, flattened to one paragraph, stripped of control characters and capped at 400 characters.
- **stderr** stays operator-only, exactly as on every other exit code.
- refusing with nothing on stdout gives the buyer a fixed "gave no reason" message rather than silence.
