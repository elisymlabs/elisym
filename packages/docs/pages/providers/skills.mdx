...
| `43` + marker | understood the request and refuses it  | none; **stdout is sent to the customer as the reason** |
| anything else | generic skill failure                  | flips the declared `(provider, model)` pair unhealthy, when the skill declares one |
...
- **stdout after the marker** is what the buyer reads, flattened to one paragraph, stripped of control characters and format marks, capped at 400 characters, and prefixed by the runtime with `The provider refused:`.
