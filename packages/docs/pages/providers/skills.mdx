...
| `43` + marker | understood the request and refuses it  | none; **the marker's line is sent to the customer as the reason** |
...
Exit `43` (`SCRIPT_EXIT_REFUSED`) is that channel, and it works for `dynamic-script`, `static-script` and `onchain`. It takes two things: the code, and stdout starting with `ELISYM-REFUSAL:`.

```sh
if [ "$unit" != "USD" ]; then
  echo "ELISYM-REFUSAL: this venue sizes positions in USD, so write it as \"size 300 USD\"."
  exit 43  # the line above reaches the buyer
fi
```

- **the marker is what decides.** 43 alone is also curl's `CURLE_BAD_FUNCTION_ARGUMENT`, which a `set -eu` script inherits from a failed request - without the marker that stays a failure, which is what it is. A marked line counts as a refusal even if the script then exits 0, so a printed refusal is never delivered to the buyer as a result.
- **the rest of the marker's line** is what the buyer reads - only that line, so anything the script prints below it stays operator-side. It is flattened to one paragraph, stripped of control characters and the format marks that reverse text, capped at 400 characters, and prefixed by the runtime with `The provider refused:`.
- **stderr** stays operator-only, exactly as on every other exit code.
- marking a refusal and saying nothing gives the buyer a fixed "gave no reason" message rather than silence.
