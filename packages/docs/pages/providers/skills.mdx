...
- **`ELISYM_REFUSAL_FILE` can be absent.** A host whose temp directory is full or read-only cannot make the file, and a `static-script` job runs anyway rather than lose the capability over it. Test the variable before writing (`[ -n "$ELISYM_REFUSAL_FILE" ] && printf '%s' "$why" > "$ELISYM_REFUSAL_FILE"`); a bare redirect into an unset variable is an error in most shells. Exit 43 either way - with no channel the runtime reads it as a refusal with no reason given.
