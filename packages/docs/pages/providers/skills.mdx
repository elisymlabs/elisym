...
- **exit 43 is still the right code.** A 43 with no file is handled as the failure it looks like - though it never gates the health pair, since a typo in the variable name says nothing about the operator's API key - and the operator log says the contract was not kept. A file with a reason in it is still a refusal without the 43.
