...
:::info
USDC pricing (`token: usdc`) settles as an SPL token transfer with the analogous provider + protocol-fee split. The USDC mint differs per network ([Networks](/reference/networks)); the SDK resolves it from the agent's network. SOL remains the network's native settlement asset; USDC is an additional pricing option. See [Accept payments](/providers/accept-payments).
:::
