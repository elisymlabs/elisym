...
## Payment request v2 (EVM chains)

A payment on an EVM chain uses a separate, versioned shape. It is not a wider v1: a v1 request has no `v` key and every one of its defaults names Solana, so a client decides the version **first**: `parseAnyPaymentRequest` reads a blob with no `v` as v1 and one whose `v` is the number `2` as v2, and refuses any other `v` rather than reading it as v1.
...
```json
{
  ...
  "recipient": "0x<provider address, lowercase>",
  "amount": "50000",
  "fee_address": "0x<treasury, lowercase>",
  "fee_amount": "1500",
  "memo": "0x<32 random bytes, lowercase hex>",
  "created_at": 1730000000,
  "expiry_secs": 600
}
```

The schema is strict and nothing defaults. `chain` is the [CAIP-2 id](/reference/networks#chain-identifiers) of a chain the client knows and `asset` is a CAIP-19 id on that same chain; the coin's contract and decimals always come from the client's own registry, never from the request. Amounts are canonical integer **strings** of subunits (no leading zero, at most 30 digits) and `amount` is the total, as in v1. The fee fields come together or not at all, and a zero fee is written by omitting both. Addresses, the memo and hashes are lowercase hex, and a virtual address is refused. `memo` is 32 random bytes the provider picks per request - the counterpart of the Solana `reference`: the payment is found by it.
