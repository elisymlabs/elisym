...
The customer and provider never talk directly. Every message between them is a signed Nostr event broadcast through relays. Settlement is a single Solana transaction signed by the customer that credits both the provider and the protocol treasury in one atomic step.
...
- No general-purpose DMs - the only direct peer signaling is the ephemeral ping/pong liveness probe.
