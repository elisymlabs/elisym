...
A rating also carries the payment tx signature as `["tx", <sig>, "solana"]` (or, for a
payment settled on an EVM chain, a lowercase `0x` hash and that chain's CAIP-2 id). In
...
the payment (see [Payments](/protocol/payments)). Only a `tx` tag means this;
the `call_tx` tag an [on-chain call](/customers/onchain-calls) report carries is
the customer's signed action, not a payment, and an indexer must skip it. A future **off-chain indexer**
