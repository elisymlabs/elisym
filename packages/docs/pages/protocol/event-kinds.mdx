...
Rating and payment-completed kind-7000 events carry a `["network", "devnet"|"mainnet"]` tag (a missing tag is read as `devnet`, so legacy events stay devnet) and a `["tx", <sig>, "solana"]` payment reference (for a payment settled on an EVM chain the last element is that chain's [CAIP-2 id](/reference/networks#chain-identifiers) and the value a lowercase `0x` hash). How ratings are counted and scoped is covered in [Reputation](/protocol/reputation).

A third kind-7000 shape reports that an [on-chain call](/customers/onchain-calls) a job returned was signed and sent. It carries `["status", "success"]` and `["call_tx", <sig>, "solana"]` - deliberately **not** `tx`, and no `rating` tag. The signature is the customer's *action*, not their payment for the job, so it carries no `elisym:v1:<jobId>` memo and must never be read as a payment reference.
