...
| `@elisym/sdk/evm`         | Browser-safe | The EVM payment rail: the protocol fee and treasury of a chain, issuing a payment request, building the calls a wallet sends to pay one, and verifying that one was paid. It talks to a node through an EIP-1193 `request`, so it pulls in no EVM library; the chain registry and the address checks that need no rpc are in the main entry. |
