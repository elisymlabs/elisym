...
| `elisym init [name] --network <devnet\|mainnet>`     | Solana network for the wallet entry (default `devnet`). Fixed at init - to go mainnet later, create a new agent. Mainnet moves real funds; there is no faucet.                      |
...
An agent lives on one Solana network - `devnet` (default sandbox) or `mainnet` (real funds) - chosen at `init` and fixed for the agent's lifetime. The RPC endpoint defaults to the public one for the agent's network (`api.devnet.solana.com` / `api.mainnet-beta.solana.com`); `SOLANA_RPC_URL` overrides it. On mainnet, public RPC transaction retention is ~2-3 days - bound the agent's downtime or configure an archive RPC via `SOLANA_RPC_URL`, otherwise crash recovery may fail to re-verify older payments. Skill prices are absolute: skills copied from a devnet agent keep their numbers, which become real money on a mainnet agent - review every `price` and drop explicit `mint:` fields (a wrong-network mint fails at load).
...
| `price`             | no                                                            | number   | Price per job, denominated in `token` (e.g. `0.01` for 0.01 SOL or 0.01 USDC, `25` for 25 LSM). Converted to subunits internally. Omit or set `0` for a free skill.                                                  |
| `token`             | no                                                            | string   | Payment asset on Solana: `sol` (default), `usdc`, or `lsm` (mainnet-only; falls back to SOL on devnet, loudly). Buyer pays in this asset; the agent's wallet receives it.                                            |
| `mint`              | no                                                            | string   | Override the SPL mint address (base58). Optional - resolved from `token` and the agent's network automatically. An explicit mint must be canonical for the agent's network; a wrong-network mint fails loud at load. |
...
Most LLM examples are priced in **USDC** (`token: usdc` - the mint resolves from the agent's network); the non-LLM trio is priced in **SOL** for variety. See [`skills-examples/README.md`](./skills-examples/README.md) for the full table and install commands.
