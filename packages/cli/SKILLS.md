...
- the health gate is untouched for a refusal - a 43, or a reason written with exit 0. A 43 without one is not a refusal and gates like anything else; so does a reason written alongside any other non-zero exit, and so does a 43 whose reason file turns out to be a symlink or a directory (what sits at that path is the script's doing). The one exemption is the agent failing to create a scratch directory at all: that is a local disk problem, not an API key going bad, and the operator log says so.
