...
- the health gate is untouched for a refusal - a 43, or a reason written with exit 0. A 43 without one is not a refusal and gates like anything else; so does a reason written alongside any other non-zero exit. The two cases where the AGENT could not give the script a channel at all (no temp directory, or something at that path it will not read) also leave health alone: that is a local disk problem, not an API key going bad, and the operator log says so.
