...
| 43 with no reason file                        | the skill meant to refuse and got the contract wrong, OR crashed with a 43 of its own (curl uses it) | same as any other failure, health gate included - the exit code cannot exempt itself; the operator log says which of the two it was                                                                                                                                                                                                                                              |
...
- refusing with an empty file still refuses, with a fixed "no reason was given" message, so it cannot be mistaken for a result. But a file holding nothing READABLE - zero bytes, a lone newline - needs the 43 to count: opening the channel early (`: > "$ELISYM_REFUSAL_FILE"`) and then succeeding must not throw away the answer the buyer paid for.
- the health gate is untouched for a refusal - a 43, or a reason written with exit 0. A 43 without one is not a refusal and gates like anything else; so does a reason written alongside any other non-zero exit, and so does a 43 whose reason file turns out to be a symlink, a hard link or a directory (what sits at that path is the script's doing). The one exemption is the job that NEVER RAN because this agent could not create its scratch directory - a local disk problem, not an API key going bad. A `static-script` skill is the exception to the exception: it runs anyway, without a channel, so a 43 from it gates like any other failure.
