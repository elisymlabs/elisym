...
| 43 with no reason file                 | the skill meant to refuse and got the contract wrong                                                | none - a copy bug in the skill says nothing about the operator's API key; the operator log says the contract was not kept                                                                                                                                  |
...
- **exit 43 is still the right exit code**, and a 43 with no file written is treated as the failure it looks like - the buyer gets the generic message - except that it never gates the health pair: a typo in the variable name is a copy bug in the skill, not evidence about the operator's API key. The operator log says the contract was not kept. A file with a REASON in it is still a refusal without exit 43: a script whose `exit 43` was swallowed by a pipeline meant what it wrote.
